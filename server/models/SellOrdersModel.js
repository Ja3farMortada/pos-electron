...
class SellOrder {
    // add order
    static async create(order, items, payment, user) {
        ...
        try {
            ...
            await connection.query(
                `INSERT INTO sales_order_items (order_id, product_id, quantity, original_price, unit_price, discount_percentage, unit_cost, avg_cost, total_price ) VALUES ?`,
                [invoice_map]
            );
            ...
            if (payment) {
                ...
                let query = `INSERT INTO journal_vouchers (journal_number, journal_date, journal_description, total_value, currency, exchange_rate) VALUES (?, ?, ?, ?, ?, ?)`;
    ...
    static async update(order, items) {
        ...
        try {
            ...
            if (journalItem) {
                wasDebt = true;
                order.operation_type = "debt";
            } else {
                order.operation_type = "normal";
            }
